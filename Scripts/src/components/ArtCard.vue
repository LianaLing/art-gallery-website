<template>
    <div class="relative group filter drop-shadow-none w-[300px]">
        ...
        <div class="absolute top-0 w-full h-full rounded-3xl backdrop-filter backdrop-brightness-100 group-hover:backdrop-brightness-75 group-hover:cursor-[zoom-in]" :class="transition"></div>
        <button class="absolute px-4 py-2 font-bold rounded-full opacity-0 bg-red hover:bg-red-hover text-light top-4 right-4 group-hover:opacity-100" :class="transition">Save</button>
        <p class="absolute bottom-4 left-4 opacity-0 group-hover:opacity-100 text-dark font-semibold bg-light rounded-full px-4 py-2" :class="transition">
            RM {{ price.toFixed(2) }}
        </p>
        <Popover class="absolute bottom-4 right-4 opacity-0 group-hover:opacity-100" :class="transition">
            <PopoverButton class="rounded-full bg-light p-2 focus:outline-none hover:bg-light-hover" :class="transition">
